...
#elif MEMS_CHIP != HW_ICM20602 && MEMS_CHIP != HW_ICM20690 && MEMS_CHIP != HW_ICM20603 && MEMS_CHIP != HW_ICM20789 && MEMS_CHIP != HW_ICM20609
...
#define RST_VAL_PWR_MGMT_1 (0x41)
    ...
    enum mpu_accel_fs
    {                  // In the ACCEL_CONFIG (0x1C) register, the full scale select  bits are :
        MPU_FS_2G = 0, // 00 = 2G
        MPU_FS_4G,     // 01 = 4
        MPU_FS_8G,     // 10 = 8
        MPU_FS_16G,    // 11 = 16
    ...
    enum mpu_accel_ois_fs
    {                      // In the ACCEL_CONFIG (0x1C) register, the accel ois full scale select bits are :
    ...
    enum mpu_accel_bw
    {                    // In the ACCEL_CONFIG2 (0x1D) register, the BW setting bits are :
    ...
    enum mpu_gyro_fs
    {                      // In the GYRO_CONFIG register, the fS_SEL bits are :
    ...
    enum mpu_gyro_bw
    {                    // In the CONFIG register, the  BW setting bits are :
